...
-- MAGIC %sql
-- MAGIC select product,category,revenue,rank() over(partition By product Order By revenue desc) as rank,
-- MAGIC dense_rank() over(partition By product Order By revenue desc) as dense_rank,
-- MAGIC ntile(2) over(partition By product Order By revenue desc) as ntile,
-- MAGIC percent_rank() over(partition By product Order By revenue desc) as p_rank,
-- MAGIC row_number() over(partition By product Order By revenue desc) as row_number
-- MAGIC from product_Revenue

-- COMMAND ----------

-- MAGIC %sql
-- MAGIC select product,category,revenue,
-- MAGIC First_value(revenue) over( Order By category desc) as first_value,
-- MAGIC last_value(revenue) over(Order By category desc) as last_value
-- MAGIC from product_Revenue 
